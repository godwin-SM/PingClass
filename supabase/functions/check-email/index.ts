...
//
// Queries public.users (which mirrors auth user emails for every onboarded
// account) rather than the auth schema. The auth schema is not exposed through
// the PostgREST API, so admin.schema("auth").from("users") silently returns
// nothing. Uses the SUPABASE_SERVICE_ROLE_KEY default secret (auto-injected by
// Supabase). Fails open (exists:false) so signup is never blocked.
